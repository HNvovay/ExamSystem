		...
		<div class="cBody">
			<div class="layui-tab" lay-filter="myPage">
				<ul class="layui-tab-title">
					<li class="layui-this" lay-id="MchooseList">多选题</li>
				</ul>
				...
				<div class="layui-tab-content">
					<div class="layui-tab-item layui-show">
						...
						<form class="layui-form" action="${pageContext.request.contextPath}/selectMchoose">
						<input type="hidden" name="judge" value="selectMchoose">
							<div class="layui-form-item">
								<div class="layui-input-inline">
									<input type="text" name="selectText"  lay-verify="required"  placeholder="输入多选题相关内容" autocomplete="off" class="layui-input">
								</div>
						...
						<form id="addPaper" class="layui-form" action="${pageContext.request.contextPath}/addPaper_Mchoose">
						<input type="hidden" name="judge" value="addPaper_Mchoose">
						<table class="layui-table">
							<thead>
								<tr>
									<th width="30px">选择</th>
									<th width="30px">id</th>
									<th width="200px">题目文本</th>
									<th width="30px">答案</th>
									<th width="80px">选项A</th>
									<th width="80px">选项B</th>
									<th width="80px">选项C</th>
									<th width="80px">选项D</th>
									<th width="80px">分数</th>
									<th>操作</th>
								</tr>
							</thead>
						...
						{	try{
							mchoose=(Mchoose)paging.getList().get(i);	
						%>
								<tr>
								<td><input type="checkbox" name="check" lay-skin="primary" value="<%=mchoose.getId() %>"></td>
									<td><%=mchoose.getId() %></td>
									<td><%=mchoose.getQuestionText()%></td>
									<td><%=mchoose.getAnswer()%></td>
									<td><%=mchoose.getOptionA() %></td>
									<td><%=mchoose.getOptionB() %></td>
									<td><%=mchoose.getOptionC() %></td>
									<td><%=mchoose.getOptionD() %></td>
									<td><%=mchoose.getScore() %></td>
									<td>
									<button type="button" class="layui-btn layui-btn-xs" onclick="updateSchoose(<%=mchoose.getId() %>)">修改</button>
									<button type="button" class="layui-btn layui-btn-xs" onclick="messageSchoose(<%=mchoose.getId() %>)">详细信息</button>
									<button type="button" class="layui-btn layui-btn-xs" onclick="deleteSchoose(this,<%=mchoose.getId() %>)">删除</button>
									</td>
								</tr>
						...
						<%}else{%>
							<a href="mchoose_list.jsp?ToPage=<%=paging.getShowPage()-1%>" ><</a>
						<%} %>
						<%for(int i=1;i<=paging.getPageCount();i++) {%>	
						<%if(paging.getShowPage()==i) {%>
							<span class="current"><%=i %></span>
							<%}else{%>			
							<a href="mchoose_list.jsp?ToPage=<%=i %>" ><%=i%></a>
							<%} %>
						<%} %>	
				...
				function deleteSchoose(_this,id){
					layer.confirm('确定删除吗', {
						...
						$(_this).parent().parent().remove()
						  location.href="${pageContext.request.contextPath}/deleteMchoose?id="+id
